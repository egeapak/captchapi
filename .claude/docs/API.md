...
**Note:** The `text` field contains the actual CAPTCHA solution generated by the server.
...
CAPTCHA_TEXT=$(echo $SESSION_RESPONSE | jq -r '.text')
```
...
curl -X POST http://localhost:3000/api/v1/sessions/$SESSION_ID/validate \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d "{\"solution\": \"$CAPTCHA_TEXT\"}"

# Response: {"valid": true/false, "session_id": "..."}
```

**Note:** Use the exact `text` value from step 2. Validation is case-sensitive.
