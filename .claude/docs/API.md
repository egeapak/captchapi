...
### Get CAPTCHA Image (JSON)

Retrieve the CAPTCHA image as a base64-encoded data URI in JSON format.

```http
GET /api/v1/sessions/{id}/image
```
...
**Behavior:**
- Solutions are compared **case-insensitively**
...
**Option A: Use binary endpoint in HTML**
```html
<img src="http://localhost:3000/api/v1/sessions/{session_id}/image.jpeg"
     alt="CAPTCHA">
```

**Option B: Use base64 data URI**
```bash
# Get base64 image
IMAGE_DATA=$(curl http://localhost:3000/api/v1/sessions/$SESSION_ID/image)

# Use in HTML
<img src="{image_data_uri}" alt="CAPTCHA">
```
