...
ok('five live fields are editable', (await p.locator('#rows input:not([disabled])').count()) === 5);
...
console.log('\n-- dirty state --');
const before = await server('captcha_compression');
...
ok('server untouched by revert', (await server('captcha_compression')) === before);

console.log('\n-- apply actually changes the server --');
await inputs.nth(0).fill('55');
await p.click('#apply');
await p.waitForSelector('#msg.ok', { timeout: 5000 });
ok('server value changed to 55', (await server('captcha_compression')) === '55');
ok('apply disabled after save', !(await p.isEnabled('#apply')));
ok('overrides line now visible', await p.isVisible('#ovr'));
ok('overrides names the field', (await p.textContent('#ovr')).includes('captcha_compression'));
...
ok('server back to its configured value', (await server('captcha_compression')) === before);
